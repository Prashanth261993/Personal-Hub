...
import { fetchMembers, fetchCategories, fetchSnapshot, createSnapshot, carryForward } from '../api';
import { dollarsToCents, centsToDollars, formatCurrency } from '@networth/shared';
import type { NetWorthEntry } from '@networth/shared';
...
type DraftEntry = {
  ...
  name: string;
  dollars: string; // user types dollars, we convert to cents on save
};
...
export default function NewSnapshot() {
  ...
  const carryFromId = searchParams.get('carryFrom');
  ...
  const { data: carryFromSnap } = useQuery({
    queryKey: ['snapshot', carryFromId],
    queryFn: () => fetchSnapshot(carryFromId!),
    enabled: !!carryFromId,
  });
  ...
  // Pre-fill from carry-forward snapshot
  useEffect(() => {
    if (carryFromSnap && draftEntries.length === 0) {
      const drafts: DraftEntry[] = carryFromSnap.entries.map((e) => ({
        key: newKey(),
        memberId: e.memberId,
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900">
        {carryFromId ? 'New Snapshot (Carried Forward)' : 'New Snapshot'}
      </h2>
          ...
          <div key={member.id} className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div className="mb-6">
              ...
              {memberAssets.length === 0 && (
                <p className="text-xs text-gray-400 italic">No assets — click "Add Asset" above</p>
              )}
            ...
            <div>
              ...
              {memberLiabs.length === 0 && (
                <p className="text-xs text-gray-400 italic">No liabilities — click "Add Liability" above</p>
              )}
              {memberLiabs.map((entry) => (
                <EntryRow
                  key={entry.key}
                  entry={entry}
                  categories={liabilityCats}
                  onUpdate={updateEntry}
                  onRemove={removeEntry}
                />
              ))}
      ...
      <div className="flex justify-end">
        <button
          onClick={handleSave}
          disabled={mutation.isPending || draftEntries.length === 0}
...
function EntryRow({
  ...
  onUpdate: (key: string, field: keyof DraftEntry, value: string) => void;
  ...
  return (
    <div className="flex items-center gap-2 mb-2">
      <select
        ...
        onChange={(e) => onUpdate(entry.key, 'categoryId', e.target.value)}
      ...
      <input
        type="text"
        value={entry.name}
        placeholder="Item name (e.g. Chase Savings)"
        onChange={(e) => onUpdate(entry.key, 'name', e.target.value)}
      ...
      <div className="relative">
        ...
        <input
          ...
          min="0"
          step="0.01"
          onChange={(e) => onUpdate(entry.key, 'dollars', e.target.value)}
      ...
      <button onClick={() => onRemove(entry.key)} className="p-2 text-gray-400 hover:text-danger-500">
