...
- **Refresh only touches the cache**: `POST /api/stocks/:id/refresh` updates the metrics cache and enriches missing metadata (company/exchange/sector/industry). It never writes metric values into the `stocks` row. `effectiveMetrics` reads straight from the cache, so the refreshed API price shows immediately. An `api-refresh` version is appended when enriched metadata changes.
...
- **Cross-fund screener**: `GET /api/funds/screener` aggregates each active fund's **latest** filing per security (via `lib/aggregation.ts`, keyed by `cusip||issuerName`), splitting value into `bullishValueCents` (shares + calls) vs `bearishValueCents` (puts) so put-heavy positions no longer inflate conviction. Each `ScreenerRow` carries `sentiment` (`bullish`/`bearish`/`mixed`/`neutral`), long-only `convictionPct`, QoQ move counts (`fundsNew`/`fundsAdded`/`fundsTrimmed`/`fundsExited`/`fundsHold`), `maxSharesChangePercent`, `netSharesChangePercent`, and a `metrics` join (`upsidePct`, `peRatio`, `pbRatio`, `psRatio`, `epsGrowth`, `marketCap` — sourced from `stock_metrics_cache`). The screener router is mounted **before** the `/:id` route so `/screener` isn't captured as an id. The client `Screener.tsx` adds search, expanded sort options, a tracked-only toggle, sentiment chips, collapsible metric/range filters, and savable presets.
- **Screener presets**: CRUD at `/api/funds/presets` (`presets.ts`, mounted before `/:id`). Presets persist to `config/funds/screener-presets.json` via `getFundScreenerPresets()`/`saveFundScreenerPresets()` in `src/lib/config.ts`. Ships 5 built-ins (`builtIn:true`, non-editable/non-deletable): Consensus Holdings, Cheap Conviction, High Upside, Big Stake Increases, Bearish Signals. User presets use `id=preset-<timestamp>`.
- **13F Insights**: `GET /api/funds/insights` (`insights.ts`, mounted before `/:id`) computes cross-fund signals on the fly (no persistence) from each active fund's latest two filings: `clusterBuys` (new+add by ≥2 funds), `newConsensus` (new in ≥2 funds), `clusterExits` (exit+trim by ≥2 funds), `bearishActivity` (any fund with put exposure), `trackedOverlap` (moves on tracked stocks), `concentratedBets` (top single-fund positions by portfolio weight), plus a representative `quarterLabel`. Client page `Insights.tsx` at `/funds/insights`.
