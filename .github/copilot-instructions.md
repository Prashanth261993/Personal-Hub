...
- **Cross-fund screener**: `GET /api/funds/screener` aggregates each active fund's **latest** filing per security (via `lib/aggregation.ts`, keyed by `cusip||issuerName`), splitting value into `bullishValueCents` (shares + calls) vs `bearishValueCents` (puts) so put-heavy positions no longer inflate conviction. Each `ScreenerRow` carries `sentiment` (`bullish`/`bearish`/`mixed`/`neutral`), long-only `convictionPct`, QoQ move counts (`fundsNew`/`fundsAdded`/`fundsTrimmed`/`fundsExited`/`fundsHold`), `maxSharesChangePercent`, `netSharesChangePercent`, and a `metrics` join (`upsidePct`, `peRatio`, `pbRatio`, `psRatio`, `epsGrowth`, `marketCap` — manual overrides win over cache). The screener router is mounted **before** the `/:id` route so `/screener` isn't captured as an id. The client `Screener.tsx` adds search, expanded sort options, a tracked-only toggle, sentiment chips, collapsible metric/range filters, and savable presets.
